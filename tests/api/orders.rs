...
async fn order_fails_when_missing_data() {
    ...
    let app = spawn_app().await;
    let test_cases = vec![
        ("item=hamburger&quantity=1&preparation_time=5", "missing table number"),
        ("table_no=1&quantity=1&preparation_time=5", "missing item"),
        ("table_no=1&item=hamburger&preparation_time=5", "missing quantity"),
        ("table_no=1&item=hamburger&quantity=1", "missing preparation time"),
    ];
